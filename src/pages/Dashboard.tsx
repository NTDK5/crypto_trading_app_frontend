  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_#0ff2,_#000_45%)] px-6 py-8 text-white">
      <div className="max-w-7xl mx-auto">
        ...
        {!isKycApproved && (
          <div className="mt-6 mb-4 rounded-2xl border border-amber-500/20 bg-amber-500/10 px-5 py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div className="text-sm text-amber-200">
              <span className="font-semibold text-amber-300">Complete identity verification</span> to unlock trading features
            </div>
            <button
              onClick={() => navigate('/app/settings/identity-verification')}
              className="px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-black text-sm font-semibold transition"
            >
              Verify Identity
            </button>
          </div>
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 md:gap-6 items-start my-6 md:my-10 lg:my-20">
          ...
          <button
            ...
            title={!isKycApproved ? 'KYC verification required' : 'Deposit'}
            className={`py-2 px-6 md:px-8 text-center rounded-xl border flex items-center gap-2 transition w-full md:w-auto justify-center ${
              !isKycApproved
                ? 'text-gray-500 border-gray-800 bg-white/[0.02] cursor-not-allowed'
                : 'text-yellow-400 border-red-200 hover:bg-yellow-400/10'
            }`}
          >
            <ArrowDown />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 my-6 md:my-10 lg:my-20">
          {['Deposit', 'Trade', 'Withdraw', 'Demo'].map(a => {
            const restricted = (a === 'Deposit' || a === 'Withdraw' || a === 'Trade') && !isKycApproved
            return (
              <button
                key={a}
                onClick={() => {
                ...
                disabled={restricted}
                title={restricted ? 'KYC verification required' : a}
                className={`flex items-center justify-center border-[1px] py-3 md:py-4 rounded-xl transition text-sm md:text-base ${
                  restricted
                    ? 'border-white/5 text-gray-600 bg-white/[0.02] cursor-not-allowed'
                    : 'border-transparent hover:bg-yellow-400/10 hover:border-yellow-300 hover:shadow-[0_0_15px_rgba(250,204,21,0.4)]'
                }`}
              >
                {actionIcons[a]}
                {a}
